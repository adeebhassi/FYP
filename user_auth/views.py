...
def sign_up(requst):
    ...
    if requst.method =="POST":
        ...
        if form.is_valid():
            ...
            user = User(username=username, email=email,password=hashed_password,email_verification_token=email_token)
            user.save()
...
def send_email_after_registration(email,email_token):
    subject='Your account needs to be verify'
    message=f'hi click the link to verify your account http://127.0.0.1:8000/user/verify/{email_token}'
